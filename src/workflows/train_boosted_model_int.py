...
def select_best_param_dict(
    DF_SubTrainInt: pd.DataFrame,
    DF_ValidationInt: pd.DataFrame,
    feature_column_list: list[str],
) -> tuple[dict, list[dict]]:
    best_param_dict = BOOSTED_PARAM_GRID[0]
    best_rmse = float("inf")
    validation_result_list: list[dict] = []

    for param_dict in BOOSTED_PARAM_GRID:
...
def main() -> dict:
    ...
    best_param_dict, validation_result_list = select_best_param_dict(
    ...
    prediction_path = build_dated_file_path(
        ...
        stem_name="boosted_prediction_int",
        date_text=run_date,
        suffix=".csv",
    )
    ...
    summary_dict = {
        ...
        "BOOSTED_PARAM_GRID": BOOSTED_PARAM_GRID,
    ...
    summary_path = build_dated_file_path(
        ...
        stem_name="boosted_model_summary_int",
        date_text=run_date,
        suffix=".json",
    )
    ...
    append_text_log(
        ...
        message_text=f"Boosted model trained -> {summary_path}",
    )
    return summary_dict
