...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run boosted walk-forward backtest on the latest feature panel."
    )
    ...
    parser.add_argument("--l2-regularization", type=float, default=None)
...
def find_latest_boosted_summary_file() -> Path:
    candidate_list = sorted(MODEL_ROOT.glob("boosted_model_summary_int_*.json"))
    if not candidate_list:
        raise FileNotFoundError("No boosted_model_summary_int file found in data/model_output.")
    return candidate_list[-1]
...
def load_best_parameter_dict() -> dict[str, float | int]:
    boosted_summary_path = find_latest_boosted_summary_file()
    boosted_summary_dict = json.loads(boosted_summary_path.read_text(encoding="utf-8"))
    best_parameter_dict = boosted_summary_dict.get("BEST_PARAMETERS")
    if not best_parameter_dict:
        raise ValueError("BEST_PARAMETERS missing from latest boosted summary.")
    return best_parameter_dict


def resolve_parameter_dict(args: argparse.Namespace) -> dict[str, float | int]:
    parameter_dict = load_best_parameter_dict()
    override_map = {
    ...
    for parameter_name, override_value in override_map.items():
        if override_value is not None:
            parameter_dict[parameter_name] = override_value
...
def run_walk_forward(
    ...
    test_window: int,
    step_size: int,
    parameter_dict: dict[str, float | int],
...
def main() -> dict:
    ...
    parameter_dict = resolve_parameter_dict(args=args)
    ...
    prediction_path = build_dated_file_path(
        ...
        stem_name="walk_forward_boosted_prediction_int",
        date_text=run_date,
        suffix=".csv",
    )
    ...
    summary_dict = {
        ...
        "MODEL_NAME": "HIST_GRADIENT_BOOSTING_REGRESSOR_WALK_FORWARD",
        "BEST_PARAMETERS": parameter_dict,
    ...
    summary_path = build_dated_file_path(
        ...
        stem_name="walk_forward_boosted_summary_int",
        date_text=run_date,
        suffix=".json",
    )
    ...
    append_text_log(
        ...
        message_text=f"Boosted walk-forward completed -> {summary_path}",
    )
    return summary_dict
